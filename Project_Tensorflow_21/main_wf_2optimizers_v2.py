...
for i in range(len(test_size_range)):
    ...
    imported_ = ImportCSVtable_inLSTM_sequence(path_CSV_feature, path_CSV_target, sequenceSize, batchSize_multiple,
                                   MinBatchMultiple, Validation_multiple, TestSize, leap, plot_histogram)
...
train_optimizer_2nd = optimizers.SGD(lr=0.01, clipnorm=1., momentum=0.9, nesterov=True)
...
# %% print out and save configuration
...
txtfile.write( \
    ...
    'TP : '+tpName+'\n'\
    '1st optimizer: '+train_optimizer_1st.__dict__['_name']+'\n'\
    'learning rate: '+str(train_optimizer_1st.__dict__['_hyper']['learning_rate'])+'\n'\
    '2nd optimizer: '+train_optimizer_2nd.__dict__['_name']+'\n'\
    'learning rate: '+str(train_optimizer_2nd.__dict__['_hyper']['learning_rate'])+'\n'\
    'activation: '+train_activation.__name__+'\n'\
    'loss function: '+train_lossfunc.__dict__['name']+'\n')
...
txtfile.write( \
    'num_epoch_1st: '+str(num_epoch_1st)+'\n'\
    'early stop patience_1st: '+str(patience_1st)+'\n'\
    'num_epoch_2nd: '+str(num_epoch_2nd)+'\n'\
    'early stop patience_2nd: '+str(patience_2nd)+'\n'\
...
model_ = create_model(minibatch_shape,
                        LSTMsize, FCsize,
                        train_optimizer_1st,
...
for i in range(start, Y_train.shape[0]-interval, interval):
    ...
    if early_stop_metric == "single metric":
        cb_trainstop = EarlyStopping(monitor='val_r_square', patience=patience_2nd,
                                   mode='max', restore_best_weights=True)
    elif early_stop_metric == "R2 rolling mean":
        cb_trainstop = create_movMetricStop_callback(stop_patience=patience_2nd,
    ...
    if (i-start) % (interval*re_intialize_intervals) == 0:
        print('Re-initializing first model')
    ...
    model_.fit(X_train_, Y_train_,
               ...
               epochs=num_epoch_1st,
               verbose=0,
               callbacks=[EarlyStopping(monitor='val_r_square', patience=patience_1st,
                                        mode='max', restore_best_weights=True),
                          cb_PrintDot()
                          ],
               shuffle=False
